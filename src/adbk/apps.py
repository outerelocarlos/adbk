...
# ReVanced apps are patched builds regenerated by ReVanced Manager (re-patch the
# base app), so the APK is reproducible rather than irreplaceable.
_REVANCED_PATTERNS = ("app.revanced.*",)

# Every built-in "never keep the APK" pattern.
_BUILTIN_SKIP_PATTERNS = _READER_EXTENSION_PATTERNS + _REVANCED_PATTERNS
...
def _matches_any(package: str, patterns: tuple[str, ...]) -> bool:
...
def is_reader_extension(package: str) -> bool:
    ...
    return _matches_any(package, _READER_EXTENSION_PATTERNS)


def _skips_apk(package: str, skip: tuple[str, ...]) -> bool:
    """Whether the package's APK should be skipped (built-in or configured).

    Built-in skips cover apps that a tool regenerates rather than distributes:
    reader extensions (reinstalled from a repo) and ReVanced patched builds.
    """

    return _matches_any(package, _BUILTIN_SKIP_PATTERNS) or _matches_any(package, skip)
