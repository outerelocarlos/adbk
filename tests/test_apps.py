...
def test_wants_apk_ignores_play_404_for_other_stores() -> None:
    # The store check only knows Google Play. A Galaxy Store app is missing there
    # (store_ok False) but is re-downloadable from its own store, so not kept...
    assert not apps.wants_apk("com.sec.android.app.samsungapps", "com.samsung.x", False, set())
    # ...while a delisted app that came from Play itself is still kept.
    assert apps.wants_apk(apps.PLAY_INSTALLER, "com.delisted", False, set())
